...
% version.
% AL 10/2016 brain voxels outside the FOV of the B0 mapping data are set to
% their value prior to unwarping (instead of 0).
...
IP = FieldMap_lren('Initialise'); % Gets default params from pm_defaults
...
%----------------------------------------------------------------------
   IP.fm = FieldMap_lren('CreateFieldMap',IP);
%----------------------------------------------------------------------
% Write out field map
% Outputs -> fpm_NAME-OF-FIRST-INPUT-IMAGE.img
%----------------------------------------------------------------------  
   fmap_img{1}=FieldMap_lren('Write',IP.P{1},IP.fm.fpm,'fpm_',64,'Smoothed phase map');
end
%----------------------------------------------------------------------
...
[IP.vdm, IP.vdmP]=FieldMap_lren('FM2VDM',IP);
...
if isfield(pm_defs, 'match_vdm')
   if pm_defs.match_vdm
          IP.vdmP = FieldMap_lren('MatchVDMxy',IP);
   end
end
IP.uepiP = FieldMap_lren('UnwarpEPIxy',IP);
...
unwarp_img{1}=FieldMap_lren('Write',IP.epiP,IP.uepiP.dat,'u',IP.epiP.dt(1),unwarp_info);
%----------------------------------------------------------------------
...
% outside the fieldmap FOV are set to 0
   ...
   unwarp_img{1+filenum}=FieldMap_lren('Write',IP.epiP,IP.uepiP.dat,'u',IP.epiP.dt(1),unwarp_info);
   fmap_img{2}=IP.vdmP;
   %IPcell{1}=IP;
